...
impl HttpEndpoint {
    ...
    /// Create the HttpEndpoint from an URL
    /// Taken from https://github.com/apoelstra/rust-jsonrpc/blob/master/src/simple_http.rs
    pub fn from_url(url: &str) -> Result<Self, std::io::Error> {
                ...
                Some(after) => {
                    ...
                    if s == "http" {
                        ...
                        return Err(std::io::Error::new(
                            ...
                            format!("scheme should be http or https ({})", url),
                        ));
                    }
                    after
                }
        ...
        if split.next().is_some() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("unexpected extra colon ({})", url),
            ));
        }
