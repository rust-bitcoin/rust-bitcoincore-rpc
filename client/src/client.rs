...
pub struct JsonOutPoint<Hash> {
    pub txid: Hash,
    pub vout: u32,
}

//impl<Hash> From<OutPoint> for JsonOutPoint<Hash> {
//    fn from(o: OutPoint) -> JsonOutPoint<Hash> {
//        JsonOutPoint {
//            txid: o.txid,
//            vout: o.vout,
//        }
...
//}
//
//impl<Hash> Into<OutPoint> for JsonOutPoint<Hash> {
...
pub trait RpcApi: Sized {
    ...
    fn get_block<Hash: Serialize>(&self, hash: &Hash) -> Result<Block> {
        let hex: String = self.call("getblock", &[into_json(hash)?, 0.into()])?;
        let bytes = hex::decode(hex)?;
        Ok(bitcoin::consensus::encode::deserialize(&bytes)?)
    }

    fn get_block_hex<Hash: Serialize>(&self, hash: &Hash) -> Result<String> {
        self.call("getblock", &[into_json(hash)?, 0.into()])
    }

    fn get_block_info<Hash: Serialize>(&self, hash: &Hash) -> Result<json::GetBlockResult> {
        self.call("getblock", &[into_json(hash)?, 1.into()])
    }
    //TODO(stevenroose) add getblock_txs

    fn get_block_header_raw<Hash: Serialize>(&self, hash: &Hash) -> Result<BlockHeader> {
        let hex: String = self.call("getblockheader", &[into_json(hash)?, false.into()])?;
        let bytes = hex::decode(hex)?;
        Ok(bitcoin::consensus::encode::deserialize(&bytes)?)
    }

    fn get_block_header_verbose<Hash: Serialize + DeserializeOwned>(
        &self,
        hash: &Hash,
    ) -> Result<json::GetBlockHeaderResult<Hash>> {
    ...
    fn get_blockchain_info<Hash: DeserializeOwned>(
        &self,
    ) -> Result<json::GetBlockchainInfoResult<Hash>> {
    ...
    fn get_best_block_hash<Hash: DeserializeOwned>(&self) -> Result<Hash> {
        self.call("getbestblockhash", &[])
    }

    /// Get block hash at a given height
    fn get_block_hash<Hash: DeserializeOwned>(&self, height: u64) -> Result<Hash> {
        self.call("getblockhash", &[height.into()])
    }

    fn get_raw_transaction<Hash: Serialize>(
        &self,
        txid: &Hash,
    ...
    fn get_raw_transaction_hex<Hash: Serialize>(
        &self,
        txid: &Hash,
        block_hash: Option<&Hash>,
    ) -> Result<String> {
    ...
    fn get_raw_transaction_verbose<Hash: Serialize>(
        &self,
        txid: &Hash,
        block_hash: Option<&Hash>,
    ...
    fn get_block_filter<Hash: Serialize>(
        &self,
        block_hash: &Hash,
    ...
    fn get_transaction<Hash: Serialize>(
        &self,
        txid: &Hash,
        include_watchonly: Option<bool>,
    ...
    fn get_tx_out_proof<Hash: Serialize>(
        &self,
        txids: &[Hash],
        block_hash: Option<&Hash>,
    ) -> Result<Vec<u8>> {
    ...
    fn lock_unspent<Hash: Serialize, OutPoint: Into<JsonOutPoint<Hash>> + Copy>(
        &self,
        outputs: &[OutPoint],
    ) -> Result<bool> {
        let outputs: Vec<_> =
            outputs.into_iter().map(|o| serde_json::to_value((*o).into()).unwrap()).collect();
        self.call("lockunspent", &[false.into(), outputs.into()])
    }

    fn unlock_unspent<Hash: Serialize, OutPoint: Into<JsonOutPoint<Hash>> + Copy>(
        &self,
        outputs: &[OutPoint],
    ) -> Result<bool> {
        let outputs: Vec<_> =
    ...
    fn generate<Hash: DeserializeOwned>(
        &self,
        block_num: u64,
        maxtries: Option<u64>,
    ) -> Result<Vec<Hash>> {
    ...
    fn invalidate_block<Hash: Serialize>(&self, block_hash: &Hash) -> Result<()> {
        self.call("invalidateblock", &[into_json(block_hash)?])
    }

    /// Mark a block as valid by `block_hash`
    fn reconsider_block<Hash: Serialize>(&self, block_hash: &Hash) -> Result<()> {
        self.call("reconsiderblock", &[into_json(block_hash)?])
    }

    /// Get txids of all transactions in a memory pool
    fn get_raw_mempool<Hash: DeserializeOwned>(&self) -> Result<Vec<Hash>> {
    ...
    fn send_raw_transaction<R: RawTx, Hash: DeserializeOwned>(&self, tx: R) -> Result<Hash> {
    ...
    /// indicates no timeout.
    fn wait_for_block<Hash: Serialize>(
        &self,
        blockhash: &Hash,
        timeout: u64,
...
mod tests {
    ...
    fn test_raw_tx() {
        ...
        assert!(client.send_raw_transaction::<_, Hash>(&tx).is_err());
        assert!(client.send_raw_transaction::<_, Hash>(&encode::serialize(&tx)).is_err());
        assert!(client.send_raw_transaction::<_, Hash>("deadbeef").is_err());
        assert!(client.send_raw_transaction::<_, Hash>("deadbeef".to_owned()).is_err());
    }
